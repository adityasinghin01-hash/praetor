...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  // Local change: a rotating cursor fights the pen-and-ink direction, where the four
  // corners are ruled square. spinDuration <= 0 turns the spin off without removing the
  // timelines, so upstream's structure (and its hover choreography) is left intact.
  const spins = spinDuration > 0;
  const spinBy = spins ? '+=360' : '+=0';
  const spinSecs = spins ? spinDuration : 1;
  ...
  useEffect(() => {
    ...
    const createSpinTimeline = () => {
      ...
      spinTl.current = gsap
        .timeline({ repeat: -1 })
        .to(cursor, { rotation: spinBy, duration: spinSecs, ease: 'none' });
    };
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current) {
            ...
            spinTl.current = gsap
              .timeline({ repeat: -1 })
              .to(cursorRef.current, { rotation: spinBy, duration: spinSecs, ease: 'none' });
            gsap.to(cursorRef.current, {
              rotation: normalizedRotation + 360,
              duration: spinSecs * (1 - normalizedRotation / 360),
              ease: 'none',
              onComplete: () => {
  ...
  return createPortal(
    <div
      ...
      // Upstream only applies cursorColor through the hover timelines, so the
      // resting cursor kept the stylesheet's white. The variable fixes the base.
      style={{ ["--target-cursor-color" as string]: cursorColor }}
    >
